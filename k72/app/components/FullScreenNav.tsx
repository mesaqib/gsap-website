"use client";

import Link from "next/link";
import {useRouter } from "next/navigation";

const FullScreenNav = ({isNavOpen, onClose}: any) => {
  const router = useRouter();
 

  return (
    <>
 

      <div className={`${isNavOpen ? "block": "hidden"} fixed h-screen w-screen bg-black text-white z-10 t-0`}>
        <div className="flex justify-between">
          <div className="p-2 w-34">
            <Link href="/">
              <svg
                className="h-full w-full"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 103 44"
              >
                <path
                  fill="white"
                  fill-rule="evenodd"
                  d="M35.1441047,8.4486911 L58.6905011,8.4486911 L58.6905011,-1.3094819e-14 L35.1441047,-1.3094819e-14 L35.1441047,8.4486911 Z M20.0019577,0.000230366492 L8.83414254,25.3433089 L18.4876971,25.3433089 L29.5733875,0.000230366492 L20.0019577,0.000230366492 Z M72.5255345,0.000691099476 L72.5255345,8.44846073 L94.3991559,8.44846073 L94.3991559,16.8932356 L72.5275991,16.8932356 L72.5275991,19.5237906 L72.5255345,19.5237906 L72.5255345,43.9274346 L102.80937,43.9274346 L102.80937,35.4798953 L80.9357483,35.4798953 L80.9357483,25.3437696 L94.3996147,25.3428482 L94.3996147,16.8953089 L102.80937,16.8953089 L102.80937,0.000691099476 L72.5255345,0.000691099476 Z M-1.30398043e-14,43.9278953 L8.78642762,43.9278953 L8.78642762,0.0057591623 L-1.30398043e-14,0.0057591623 L-1.30398043e-14,43.9278953 Z M58.6849955,8.4486911 L43.1186904,43.9274346 L52.3166592,43.9274346 L67.9877996,8.4486911 L58.6849955,8.4486911 Z M18.4688864,25.3437696 L26.7045278,43.9278953 L36.2761871,43.9278953 L28.1676325,25.3375497 L18.4688864,25.3437696 Z"
                ></path>
              </svg>
            </Link>
          </div>

          <div className="relative cursor-pointer h-[10vh] w-[10vw]" onClick={onClose}>
            <div>
              <span className="absolute top-8 h-[.3vh] rounded-full right-10 w-16 bg-white transform origin-center -rotate-45"></span>
              <span className="absolute top-8 h-[.3vh] rounded-full right-10 w-16 bg-white transform rotate-45 origin-center"></span>
            </div>
          </div>
        </div>

        <div className="flex flex-col items-center justify-start h-full font-[heading1] mt-12">
          <div
            className="navLink w-full relative h-[16vh] cursor-pointer"
            onClick={() => router.push("/project")}
          >
            <div className="border-t text-center">
              <h1 className="uppercase text-[7vw]  mt-[2vh]  leading-22 border-b ">
                Project
              </h1>
            </div>

            <div className="navHover absolute top-0 flex h-full w-full overflow-hidden bg-[#D3FD50]">
              <div className="navMove flex w-max shrink-0 items-center gap-8 whitespace-nowrap text-[7vw] uppercase">
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
              </div>
            </div>
          </div>

          <div
            className="navLink w-full relative h-[16vh] cursor-pointer"
            onClick={() => router.push("/project")}
          >
            <div className="border-t text-center">
              <h1 className="uppercase text-[7vw]  mt-[2vh]  leading-22 border-b ">
                agence
              </h1>
            </div>

            <div className="navHover absolute top-0 flex h-full w-full overflow-hidden bg-[#D3FD50]">
              <div className="navMove flex w-max shrink-0 items-center gap-8 whitespace-nowrap text-[7vw] uppercase">
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
              </div>
            </div>
          </div>

          <div
            className="navLink w-full relative h-[16vh] cursor-pointer"
            onClick={() => router.push("/project")}
          >
            <div className="border-t text-center">
              <h1 className="uppercase text-[7vw]  mt-[2vh]  leading-22 border-b ">
                contact
              </h1>
            </div>

            <div className="navHover absolute top-0 flex h-full w-full overflow-hidden bg-[#D3FD50]">
              <div className="navMove flex w-max shrink-0 items-center gap-8 whitespace-nowrap text-[7vw] uppercase">
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
              </div>
            </div>
          </div>

          <div
            className="navLink w-full relative h-[16vh] cursor-pointer"
            onClick={() => router.push("/project")}
          >
            <div className="border-t text-center">
              <h1 className="uppercase text-[7vw]  mt-[2vh]  leading-22 border-b ">
                blogue
              </h1>
            </div>

            <div className="navHover absolute top-0 flex h-full w-full overflow-hidden bg-[#D3FD50]">
              <div className="navMove flex w-max shrink-0 items-center gap-8 whitespace-nowrap text-[7vw] uppercase">
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
                <span> Pour Tout voir</span>
                <img
                  src="/media/img/nav1.png"
                  className="object-fit h-[7vw] w-[20vw] rounded-full"
                />
              </div>
            </div>
          </div>
        </div>
      </div>

    </>
  );
};

export default FullScreenNav;
